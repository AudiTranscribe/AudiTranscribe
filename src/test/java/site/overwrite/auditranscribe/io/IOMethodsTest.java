...
class IOMethodsTest {
    ...
    void createFile() {
        ...
        assertEquals(0, IOMethods.createFile(IOConstants.APP_DATA_FOLDER_PATH_STRING + "testFile.txt"));

        // Attempt to create the same file again should return 1
        assertEquals(1, IOMethods.createFile(IOConstants.APP_DATA_FOLDER_PATH_STRING + "testFile.txt"));
    }
    ...
    void deleteFile() {
        ...
        assertTrue(IOMethods.deleteFile(IOConstants.APP_DATA_FOLDER_PATH_STRING + "testFile.txt"));

        // Second time round, the file could not be deleted and thus returns `false`
        assertFalse(IOMethods.deleteFile(IOConstants.APP_DATA_FOLDER_PATH_STRING + "testFile.txt"));
    }
}
...
//    /**
//     * Compresses the provided bytes using the DEFLATE algorithm.
//     *
//     * @param bytes Bytes to be compressed.
//     * @return Compressed bytes.
//     * @see <a href="https://en.wikipedia.org/wiki/Deflate">The DEFLATE algorithm</a>.
//     */
//    public static byte[] deflateCompress(byte[] bytes) {
//        // Assume (more than) worst case scenario and assign more bytes than required
//        // (See: https://www.w3.org/Graphics/PNG/RFC-1951, §1.1; we assume 0.05% increase as the worst case)
//        int worstCaseLength = (int) Math.ceil(1.0005 * bytes.length);
//        byte[] tempOutput = new byte[worstCaseLength];
//
//        // Actually compress the bytes
//        Deflater compressor = new Deflater();
//        compressor.setInput(bytes);
//        compressor.finish();
//        int compressedLength = compressor.deflate(tempOutput);
//        compressor.end();
//
//        // Reduce output size to actual compressed length
//        byte[] output = new byte[compressedLength];  // Now we fill in actual output array
//        System.arraycopy(tempOutput, 0, output, 0, compressedLength);
//        return output;
//    }
//
//    /**
//     * Decompresses the provided bytes using the DEFLATE algorithm.
//     *
//     * @param compressedBytes Bytes to be decompressed.
//     * @param originalLength  Length of the decompressed array of bytes.
//     * @return Decompressed bytes.
//     * @throws DataFormatException If the compressed bytes are not in the correct format.
//     * @see <a href="https://en.wikipedia.org/wiki/Deflate">The DEFLATE algorithm</a>.
//     */
//    public static byte[] deflateDecompress(byte[] compressedBytes, int originalLength) throws DataFormatException {
...
//
//    /**
//     * Decompresses the provided bytes using the DEFLATE algorithm.<br>
//     * Fails silently, so no exception is thrown.
//     *
//     * @param compressedBytes Bytes to be decompressed.
//     * @param originalLength  Length of the decompressed array of bytes.
...
//     */
//    @ExcludeFromGeneratedCoverageReport
//    public static byte[] deflateDecompressFailSilently(byte[] compressedBytes, int originalLength) {
//        try {
//            return deflateDecompress(compressedBytes, originalLength);
